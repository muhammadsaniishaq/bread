      ...
      <div style={{ minHeight: '100vh', background: T.bg, paddingBottom: '100px', fontFamily: "'Inter', system-ui, sans-serif" }}>
        ...
        <div style={{ background: '#0a0f1d', padding: '32px 20px 48px', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'relative', zIndex: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
               <h1 style={{ fontSize: '18px', fontWeight: 900, color: '#fff', margin: 0 }}>Lissafin Supplier</h1>
            ...
            <p style={{ margin: 0, fontSize: '11px', color: 'rgba(255,255,255,0.4)', fontWeight: 600 }}>Pure Supplier Management • No Customers</p>
          </div>
        </div>
        ...
        <div style={{ padding: '0 16px', marginTop: '-24px', position: 'relative', zIndex: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 0.8fr 1fr', gap: '8px', marginBottom: '20px' }}>
             {[
               { label: 'Bashin Supliers', val: fmt(totalDebt), icon: Wallet, color: '#f87171' },
               { label: 'Requests', val: pendingCount, icon: Clock, color: '#fbbf24' },
               { label: 'An Biya', val: fmt(totalPaid), icon: CheckCircle2, color: '#34d399' }
             ].map((s, i) => (
          ...
          <div style={{ display: 'flex', background: 'rgba(0,0,0,0.04)', padding: '4px', borderRadius: '16px', marginBottom: '20px' }}>
             {(['LEDGER', 'REQUESTS'] as const).map(t => (
               <button key={t} onClick={() => setTab(t)}
                 style={{ flex: 1, padding: '10px', borderRadius: '12px', border: 'none', background: tab === t ? '#fff' : 'transparent', color: tab === t ? T.primary : T.txt3, fontSize: '12px', fontWeight: 900, cursor: 'pointer', boxShadow: tab === t ? '0 2px 8px rgba(0,0,0,0.05)' : 'none' }}>
                  {t === 'LEDGER' ? 'Littafin Bashi' : `Bukatun (${pendingCount})`}
               </button>
             ))}
          </div>
          ...
          {tab === 'LEDGER' ? (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {loading ? <div style={{ textAlign: 'center', padding: '30px', fontSize: '12px', color: T.txt3 }}>Aiki ake...</div> : (
                  <>
                    {filteredSuppliers.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '40px 20px', background: '#fff', borderRadius: '16px', border: `1px dashed ${T.border}` }}>
                        <p style={{ margin: 0, color: T.txt3, fontSize: '12px', fontWeight: 700 }}>Babu Supplier anan</p>
                      </div>
                    ) : (
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                             <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                <div>
                                   ...
                                   <div style={{ fontSize: '9px', fontWeight: 700, color: T.txt3 }}>{s.unitsTaken} Units</div>
                                </div>
                             </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
               {myPendingRequests.length === 0 ? (
                 <div style={{ textAlign: 'center', padding: '54px 24px', background: '#fff', borderRadius: '20px', border: `1px dashed ${T.border}` }}>
                    ...
                    <h3 style={{ margin: 0, color: T.ink, fontSize: '14px', fontWeight: 900 }}>Babu wani aiki yanzu</h3>
                 </div>
               ) : (
                      ...
                      <motion.div key={tx.id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                           <div style={{ display: 'flex', gap: '10px' }}>
                              <div style={{ width: '32px', height: '32px', borderRadius: '10px', background: tx.type === 'Return' ? 'rgba(239,68,68,0.1)' : 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                 {tx.type === 'Return' ? <ArrowUpRight size={14} color={T.danger} /> : <ArrowDownLeft size={14} color={T.success} />}
                              </div>
                              <div>
                                 ...
                                 <div style={{ fontSize: '10px', fontWeight: 800, color: tx.type === 'Return' ? T.danger : T.success }}>
                                   {tx.type === 'Return' ? 'Maida Kaya' : 'Karbar Kaya'}
                                 </div>
                              </div>
                           </div>
                           <div style={{ textAlign: 'right' }}>
