        ...
        <div style={{ background: 'linear-gradient(145deg, #0a1628 0%, #0f2952 50%, #1e40af 100%)', padding: '48px 20px 28px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
              ...
              <div>
                <h1 style={{ fontSize: '20px', fontWeight: 900, color: '#fff', margin: 0 }}>{t('store.liveStock')}</h1>
                <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', margin: 0, fontWeight: 600 }}>{t('store.liveStockPreview')}</p>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
              {[
                { label: t('store.unitsTotal'), value: totalStock, color: '#93c5fd' },
                { label: t('store.received'), value: totalReceivedToday, color: '#6ee7b7' },
                { label: t('store.dispatched'), value: totalSoldToday, color: '#fde68a' },
                { label: t('store.lowStock'), value: lowItems.length, color: lowItems.length > 0 ? '#fca5a5' : '#6ee7b7' },
              ].map((s, i) => (
            ...
            <div style={{ position: 'relative', marginTop: '14px' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('store.searchProduct')}
        ...
        <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          {outItems.length > 0 && (
            <div style={{ padding: '12px 14px', borderRadius: '14px', background: T.roseL, border: `1px solid ${T.rose}25`, display: 'flex', gap: '10px', alignItems: 'center' }}>
              ...
              <div>
                <div style={{ fontSize: '12px', fontWeight: 800, color: T.rose }}>{t('store.outOfStock')}</div>
                <div style={{ fontSize: '11px', color: T.txt2 }}>{outItems.map(p => p.name).join(', ')} — {t('store.restockImmediately')}</div>
              </div>
            </div>
          )}
          ...
          {lowItems.length > 0 && outItems.length === 0 && (
            <div style={{ padding: '12px 14px', borderRadius: '14px', background: T.amberL, border: `1px solid ${T.amber}25`, display: 'flex', gap: '10px', alignItems: 'center' }}>
              ...
              <div style={{ fontSize: '12px', fontWeight: 800, color: T.amber }}>⚠️ {lowItems.length} {t('store.lowStock')}</div>
            </div>
          )}
          ...
          {outItems.length === 0 && lowItems.length === 0 && (
            <div style={{ padding: '10px 14px', borderRadius: '12px', background: T.emeraldL, border: `1px solid ${T.emerald}25`, display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: 700, color: T.emerald }}>{t('store.allHealthly')} ✅</span>
            </div>
          )}
          ...
          {chartData.length > 0 && (
            <div style={{ background: T.white, borderRadius: T.radius, padding: '16px', boxShadow: T.shadow, border: `1px solid ${T.borderL}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: 800, color: T.ink }}>{t('store.liveStockPreview')}</span>
              </div>
              <div style={{ height: '140px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} barCategoryGap="30%" margin={{ top: 4, right: 4, bottom: 0, left: -24 }}>
                    ...
                    <Bar dataKey="stock" name={t('store.stockLeft')} radius={[4, 4, 0, 0]}>
                      {chartData.map((d, i) => <Cell key={i} fill={d.isLow ? T.rose : T.primary} fillOpacity={0.85} />)}
                    </Bar>
                    <Bar dataKey="sold" name={t('store.dispatched')} fill={T.amber} radius={[4, 4, 0, 0]} fillOpacity={0.75} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
              <div style={{ display: 'flex', gap: '14px', justifyContent: 'flex-end', marginTop: '6px' }}>
                {[
                  { color: T.primary, label: t('store.unitsLeft') },
                  { color: T.amber, label: t('store.dispatched') },
                  { color: T.rose, label: t('store.lowStock') },
                ].map((l, i) => (
          ...
          {/* Product cards */}
          <div style={{ background: T.white, borderRadius: T.radius, padding: '16px', boxShadow: T.shadow, border: `1px solid ${T.borderL}` }}>
            <div style={{ fontSize: '13px', fontWeight: 800, color: T.ink, marginBottom: '12px' }}>{t('store.allProducts')}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {activeProducts.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '24px', color: T.txt3, fontSize: '13px' }}>{t('store.noRecordsFound')}</div>
                  ...
                  <motion.div key={p.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ fontSize: '9px', color: T.txt3, fontWeight: 700 }}>{t('store.unitsLeft')}</div>
                      </div>
                    </div>
                    {/* Progress bar */}
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '7px' }}>
                      {sold > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          ...
                          <span style={{ fontSize: '10px', color: T.rose, fontWeight: 700 }}>-{sold} {t('store.dispatched')}</span>
                        </div>
                      )}
                      {received > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <TrendingUp size={11} color={T.emerald} />
                          <span style={{ fontSize: '10px', color: T.emerald, fontWeight: 700 }}>+{received} {t('store.received')}</span>
                        </div>
                      )}
                      {isOut && <span style={{ fontSize: '10px', color: T.rose, fontWeight: 800 }}>🚫 {t('store.outOfStock')}</span>}
                      {isLow && !isOut && <span style={{ fontSize: '10px', color: T.amber, fontWeight: 800 }}>⚠️ {t('store.lowStock')}</span>}
                    </div>
                  </motion.div>
                );
              })}
