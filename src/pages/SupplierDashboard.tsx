      ...
      <div style={{ minHeight: '100vh', background: T.bg, paddingBottom: '80px', fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ background: '#0a0f1d', padding: '32px 20px 48px', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
             <h1 style={{ fontSize: '18px', fontWeight: 900, color: '#fff', margin: 0 }}>Sannu, {user?.user_metadata?.full_name?.split(' ')[0]}</h1>
          ...
          <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '20px', padding: '16px', border: '1px solid rgba(255,255,255,0.1)' }}>
             <p style={{ margin: 0, fontSize: '10px', fontWeight: 800, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase' }}>Bashin da ake binka</p>
        ...
        <div style={{ padding: '0 16px', marginTop: '-24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '24px' }}>
             {[
               { id: 'RECEIVE', label: 'Karba', sub: 'Kaya', icon: Plus, color: T.primary, bg: T.pLight },
               { id: 'RETURN', label: 'Maida', sub: 'Kaya', icon: RefreshCw, color: T.danger, bg: 'rgba(239,68,68,0.05)' },
               { id: 'PAYMENT', label: 'Biya', sub: 'Bashi', icon: CreditCard, color: T.success, bg: 'rgba(16,185,129,0.05)' }
             ].map((a) => (
          ...
          {pendingTxs.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
               <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px', paddingLeft: '4px' }}>
                  ...
                  <span style={{ fontSize: '11px', fontWeight: 900, color: T.ink, textTransform: 'uppercase' }}>Jiran Tabbatarwa ({pendingTxs.length})</span>
               </div>
                  ...
                  {pendingTxs.map(tx => (
                    <div key={tx.id} style={{ background: '#fff', borderRadius: '16px', padding: '10px 14px', border: `1px solid ${T.amber}20`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                       <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                          <div style={{ fontSize: '12px', fontWeight: 800, color: T.ink }}>{tx.type === 'Return' ? 'Maida Kaya' : 'Karbar Kaya'}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px', paddingLeft: '4px' }}>
             ...
             <span style={{ fontSize: '11px', fontWeight: 900, color: T.ink, textTransform: 'uppercase' }}>Tarihin Aiki</span>
          </div>
                 ...
                 style={{ width: '100%', background: '#fff', borderRadius: '24px 24px 0 0', padding: '24px', boxShadow: '0 -10px 40px rgba(0,0,0,0.1)' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                     <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 900, color: T.ink }}>
                        {showModal === 'RECEIVE' ? 'Bukata Kaya' : showModal === 'RETURN' ? 'Maida Kaya' : 'Biya Bashi'}
                     </h2>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                     {showModal === 'PAYMENT' ? (
                       <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '16px', border: `1px solid ${T.border}` }}>
                          <label style={{ fontSize: '10px', fontWeight: 800, color: T.txt3, textTransform: 'uppercase', marginBottom: '8px', display: 'block' }}>Kudin da zaka bayar</label>
                       ...
                       <>
                         <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            ...
                            <input type="number" placeholder="Yawan Kaya (pcs)" value={qty} onChange={(e) => setQty(e.target.value)}
                     ...
                     <motion.button whileTap={{ scale: 0.97 }} onClick={handleSubmit} disabled={submitting || (showModal === 'PAYMENT' ? !payAmount : (!selectedProd || !qty || !selectedSK))}
                       style={{ width: '100%', padding: '16px', borderRadius: '16px', background: T.primary, color: '#fff', border: 'none', fontSize: '15px', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                        {submitting ? 'Aiki ake...' : <><CheckCircle size={18} /> Tura Bukata</>}
                     </motion.button>
                  </div>
               </motion.div>
            </motion.div>
